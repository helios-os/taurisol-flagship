...
import { getArticleData, getCategoryByFiSlug } from "@/lib/journal";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = await getArticleData(categorySlug, articleSlug, "fi");
  if (!article) return { title: "Ei löydy — Taurisol Journal" };
  return {
...
export default async function FiArticlePage({ params }: Props) {
  const { categorySlug, articleSlug } = await params;
  const article = await getArticleData(categorySlug, articleSlug, "fi");
  if (!article) notFound();

  const category = getCategoryByFiSlug(article.categorySlug) ??
    getCategoryByFiSlug(categorySlug);
              ...
              <div className="mx-auto max-w-3xl px-6 md:px-12">
                <Reveal>
                  <p className="font-serif text-xl italic text-shadow/50">
                    Artikkelin sisältö on valmistelussa.
                  </p>
                </Reveal>
              </div>
            </section>
          ...
          <section className="bg-sand border-t border-stone-warm/40 py-16">
            <div className="mx-auto max-w-7xl px-6 md:px-12">
              ...
              >
                ...
                Takaisin: {category?.title.fi ?? "Journal"}
              </a>
            </div>
          </section>
        </main>
