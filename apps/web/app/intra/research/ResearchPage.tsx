...
export default function ResearchPage() {
  const { lang } = useLang();
  const c = content.intra.research;
    ...
    <IntraShell eyebrow={t(c.eyebrow, lang)} title={t(c.title, lang)} intro={t(c.intro, lang)}>
      <IntraSection>
        <div className="grid gap-12 md:gap-16 lg:grid-cols-2">
          <IntraBlock
            heading={t(c.themesHeading, lang)}
            items={c.themesItems.map((it) => t(it, lang))}
          />
          <IntraBlock
            heading={t(c.offerHeading, lang)}
            items={c.offerItems.map((it) => t(it, lang))}
          />
        </div>
      </IntraSection>
      ...
      <IntraSection tone="sand">
        <div className="grid gap-10 lg:grid-cols-[1.2fr_1fr] lg:gap-16 items-start">
          ...
          <div className="flex flex-col gap-4 sm:flex-row lg:flex-col lg:items-end">
            <IntraRequestDialog defaultPath="research">
              <PrimaryCTA>{t(c.submitCta, lang)}</PrimaryCTA>
            </IntraRequestDialog>
            <IntraRequestDialog defaultPath="research" variant="founding">
